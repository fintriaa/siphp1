...
$(document).ready(function () {
     function ubahTanggal() {
          ...
          let tglTarget = document.querySelectorAll('#tgl-target-tabel');
          let tglRealisasi = document.querySelectorAll('#tgl-realisasi-tabel');
          console.log(tglTabel);
          for (i = 0; i <= tglTabel.length; i++) {
               tglTabel[i].innerHTML = ubahFormatTanggal2(tglTabel[i].textContent);
               tglTarget[i].innerHTML = ubahFormatTanggal2(tglTarget[i].textContent);
               tglRealisasi[i].innerHTML = ubahFormatTanggal2(tglRealisasi[i].textContent);
          }
     }
...
$(document).on('input', '#formFileMultiple', function () {
     if (this.files[0].size > 1000000) {
          // ini untuk ukuran 1MB
          Toast.fire({
               icon: 'warning',
               title: 'Total Ukuran File Melebihi 1Mb!',
          });
          this.value = '';
          return false;
     }
...
$('#btn-modal-tambah-master-pkj').click(function () {
     $('#modal-tambah').modal('show');
});
